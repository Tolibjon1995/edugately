...
export default function SettingsRightside() {
  ...
  const getSelectedFaculty = async () => {
    try {
      const res = await Axios.get(
        `/university/faculty/?university_id=${certainId}&degree=${certainDegree}&limit=${rowsPerPage}`
      );
  ...
  const handlePageChange = async (e, newPage) => {
    ...
    setLoading(true)
    try {
      const res = await Axios.get(`/university/faculty/?limit=${rowsPerPage}&offset=${newPage * rowsPerPage}`);
      const { status, data } = res;
      const { results } = data;
      if (status == 200) {
        setSelectFaculty(results);
  ...
  const getAll =async()=>{
    try {
      const res = await Axios.get(
        `/university/faculty/?limit=${rowsPerPage}`
      );
                    ...
                    selectedFaculty.map((item, i) => {
                      ...
                      return (
                        <tr key={id}>
                          ...
                          <td className="inovice-row">{service_price}</td>
