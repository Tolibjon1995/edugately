...
function SidebarAccountant(props) {
  ...
  const [avearage, setAvearage] = useState(0);
  ...
  const fethcStudents = async () => {
    try {
      ...
      if (status === 200) {
        if (!localStorage.getItem("newAccountCount")) {
          localStorage.setItem("newAccountCount", count)
        }
        const data = count - localStorage.getItem("newAccountCount");
        setAvearage(count)
        setCount(data)
        localStorage.setItem("newAccountCount", count);
        sessionStorage.setItem("newAccountCount", count);
      }
    } catch (error) {
    }
  };
  ...
  const signOut = async () => {
    ...
    history.replace("/");
    localStorage.clear();
    localStorage.setItem("newAccountCount", avearage);
    // try{
        ...
        <div className="dashboardBlock">
          <div className="sidebarFixed">
            ...
            <div className="sidebar">
              ...
              <NavLink onClick={handleclose} to="/accountant-check">
                To'lov tasdiqlanmaganlar 
              </NavLink>
