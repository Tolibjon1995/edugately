...
function SinglePage(props) {
  ...
  const postSelectedUniver = async () => {
    if (!selector?.payload?.payload?.data?.id) {
      ...
      selector?.payload?.payload?.data?.passport_number
    ...
    <React.Fragment>
      ...
      <div className="container single-univer ">
        ...
        <div className="row university-offer">
          <div className="col-12">
            <h1 className="title">{t('singleunver1')}</h1>
          </div>
        </div>
        ...
        <div className="row info">
          <div className="col-12 p-3 bg-white bgr">
            <div className="table-responsive">
              <table className="table table-hover">
                <thead>
                  <tr>
                    <th className="h4"> {t("p241")}</th>
                    <th className="h4"> {t("p438")}</th>

                    <th className="h4"> {t("part53")}</th>

                    <th className="h4"> {t("p336")}</th>
                  </tr>
                </thead>
                <tbody>
                  {faculties.map((f) => {
                      ...
                      <tr>
                        <td >
                          <span
                            ...
                            style={{ color: "#325CFD", cursor: "pointer" }}
                          >
                            {i18n.language == 'uz' ? f.name_uz : ''}
                            {i18n.language == 'ru' ? f.name_ru : ''}
                            {i18n.language == 'en' ? f.name_en : ''}
                          </span>
                        </td>
                        <td className="h4">{f.quota} </td>

                        <td className="h4">{f.degree_name}</td>
