...
const UnverList2 = () => {
    ...
    useEffect(() => {
        axios.get(`https://backend.edugately.com/api/v1/university/uzbuniversity/?limit=10000`).then((res) => {
            setUnver(res.data.results);
        })
    ...
    return (
        <div>
            {/* <Navbar /> */}
            <div className="unverContainer">
                <div className="unverSidebar">
                    <NavLink to="/">
                    ...
                    <h3 className='mt-80'>Universitetlar filteri</h3>
                    <div className="kvota">
                        <SearchIcon onClick={() => { searchUnver() }} />
                        <input type="text" onClick={() => { searchUnver() }} onChange={(e) => { setSearch(e.target.value) }} placeholder='Universitet nomini kiriting' />
                    </div>
                    <div className="kvota mt-80">
                        <SearchIcon onClick={() => { searchUnver2() }} />
                        <input type="text" onClick={() => { searchUnver2() }} onChange={(e) => { setSearch2(e.target.value) }} placeholder='Joy nomini kiriting' />
                    </div>
                    <div className="kvota mt-80">
                        <SearchIcon onClick={() => { searchUnver3() }} />
                        <input type="text" onClick={() => { searchUnver3() }} onChange={(e) => { setSearch3(e.target.value) }} placeholder="Yo'nalish nomini kiriting" />
                    </div>
                </div>
                <div className="unverContent">
                                    ...
                                    unver?.map((item, index) => {
                                        return (
                                            <div
                                                key={index}
                                                className="new-card"
                                                onClick={() => history.push(`/uzbuniversity/${item?.id}/`)}
                                            >
                                                <div className="new-card-container">
                                                    <div className="mew-card-img">
                                                ...
                                                {/* <a className='' href={item.site}>{item.site}</a> */}
                                            </div>
                                        )
                                    })
                                }
