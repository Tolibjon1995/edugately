...
const M_doc_rec_send = () => {
  ...
  const datas2 = {
    ...file,
    next_step : 'manager_reject_notary',
  ...
  const sendNotary = async () => {
    try {
      setWhereGoFile(false)
      // if (whereGoFile === false) {
      //   console.log('1 ',next_step);
        const res = await Axios.patch(
          `/applicant/manager-check-documents/${params.id}/`,
          datas2
        );
        ...
        <div className="doc_box">
          ...
          >
            <div className="doc_btn">
              <button onClick={sendUniver}> {t("p465")}</button>
            </div>
            <div className="doc_btn">
              <button onClick={handleOpenx}> {t("p4662")}</button>
            </div>
          </div>
        </div>
        ...
        >
          <Fade in={openx}>
            <div className="modal2">
              ...
              <img src={idea} alt="" />
              <h1> {t("p467")}</h1>
              <input type="text" style={{ color: '#FFF' }} placeholder="Напишите причину отказа" />
              <div className="modal_btn">
                <button onClick={handleClosex}>{t("p247")}</button>
                <button onClick={sendNotary}> {t("p249")}</button>
              </div>
            </div>
          </Fade>
        </Modal>
